...
class Grid2DNetwork(RNNLayers):
    ...
    def set_layers(self, unit, depth, n_in, n_h):
        ...
        layers = []
        for i in xrange(depth):
            layers.extend([layer(n_in=n_h, n_h=n_h) for i in xrange(2)])
            layers.append(Layer(n_in=n_h * 2, n_h=n_h))
        return layers
    ...
    def grid_propagate(self, h):
        ...
        :return: 1D: batch, 2D: n_prds, 3D: n_words, 4D: dim_h
        """
        h0_lr = T.zeros((h.shape[0], h.shape[1], h.shape[3]), dtype=theano.config.floatX)
        h0_ud = T.zeros((h.shape[0], h.shape[2], h.shape[3]), dtype=theano.config.floatX)
        for i in xrange(0, self.depth):
    ...
    @staticmethod
    def dot(layer, h_lr, h_ud):
        h_lr = h_lr.dimshuffle(1, 2, 0, 3)
        h_ud = h_ud.dimshuffle(1, 0, 2, 3)
        h = T.concatenate([h_lr, h_ud], axis=3)
        return layer.dot(h)
